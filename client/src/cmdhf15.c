...
static int CmdHF15Inventory(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 15 inventory",
                  "Perform ISO-15693 inventory to find tags in the field.\n"
                  "Supports 1-slot and 16-slot modes.\n"
                  "In 16-slot mode, performs full anti-collision tree walking\n"
                  "to discover all tags.",
                  "hf 15 inventory\n"
                  "hf 15 inventory --slot16\n"
                  "hf 15 inventory --slot16 --afi 01\n"
                 );

    void *argtable[] = {
        arg_param_begin,
        arg_lit0(NULL, "slot16", "use 16-slot mode (default: 1-slot)"),
    ...
    bool slot16 = arg_get_lit(ctx, 1);

    int afi_len = 0;
    uint8_t afi_buf[1] = {0};
    CLIGetHexWithReturn(ctx, 2, afi_buf, &afi_len);

    bool fast = (arg_get_lit(ctx, 3) == false); // default is fast

    CLIParserFree(ctx);

    if (slot16 == false) {
        // Simple 1-slot inventory (same as existing reader behavior)
        uint8_t raw_cmd[8] = {0};
        uint16_t raw_len = 0;
        ...
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(WARNING, "Inventory failed (%d)", res);
            return res;
        }

        if (num_found > 0 && results[0].status == 1 && results[0].data_len >= 12) {
            uint8_t uid[8];
            memcpy(uid, results[0].data + 2, 8);

            PrintAndLogEx(SUCCESS, "UID: " _GREEN_("%s") "  DSFID: %02X",
                          iso15693_sprintUID(NULL, uid), results[0].data[1]);
        } else {
            PrintAndLogEx(WARNING, "No tag found");
        }
    ...
    // 16-slot mode with anti-collision tree walking
    // Work queue: each entry is a mask (up to 8 bytes) and mask_len in bits
    typedef struct {
        uint8_t mask[8];
    ...
    while (work_head < work_tail && work_head < MAX_WORK_ITEMS) {
        ...
        // Keep field on if there are more work items to process
        bool more_work = (work_head < work_tail);
        int res = perform_iso15_inventory(raw_cmd, raw_len, true, fast, more_work, results, &num_found);
        if (res != PM3_SUCCESS) {
...
static command_t CommandTable[] = {
    ...
    {"inventory",           CmdHF15Inventory,         IfPm3Iso15693,   "Inventory ISO-15693 tags (1 or 16 slots)"},
